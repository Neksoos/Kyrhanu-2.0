...
-- ✅ prevent duplicate season seeds by name
CREATE UNIQUE INDEX IF NOT EXISTS ux_seasons_name ON seasons(name);
...
-- ✅ prevent duplicate event seeds (identity by type+name)
CREATE UNIQUE INDEX IF NOT EXISTS ux_live_events_type_name ON live_events(event_type, name);

-- ✅ prevent duplicate boss seeds by name
CREATE UNIQUE INDEX IF NOT EXISTS ux_live_bosses_name ON live_bosses(name);
...
-- ============================================================
-- Seed content for Cursed Mounds (idempotent)
...
INSERT INTO seasons (name, theme, starts_at, ends_at, bp_base_price, bp_premium_price, is_active)
VALUES
...
INSERT INTO live_events (event_type, name, description, config, starts_at, ends_at, is_active)
VALUES
(
    'double_drop',
    'Подвійна Доля',
