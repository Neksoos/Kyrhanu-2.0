...
class AntiCheatService:
    ...
    async def validate_tap(
        ...
        # 1. Check HMAC authenticity
        if not self._verify_hmac(user_id, sequence_number, client_timestamp, nonce, hmac_signature):
            return TapValidationResult(
                is_valid=False,
                reason="Invalid HMAC signature",
                anomaly_delta=25.0,
                new_anomaly_score=min(100.0, current_anomaly_score + 25.0)
            )
        ...
        # 4. Sequence number check (prevent replay)
        ...
        # 5. Behavioral analysis - timing patterns
        ...
        # 6. Check for rapid-fire (autoclicker)
    ...
    def _verify_hmac(
        ...
        """Verify HMAC signature of action."""
        if not settings.ANTI_CHEAT_ENABLED:
            return True  # Dev bypass
            
        message = f"{user_id}:{sequence}:{timestamp}:{nonce}"
        expected = hmac.new(
