...
from routers.auth import get_tg_id as current_tg_id
...
# ✅ гравець з players
async def _get_player_brief(conn, tg_id: int) -> Dict[str, Any]:
    try:
        row = await conn.fetchrow(
            "SELECT name, level FROM players WHERE tg_id=$1",
            int(tg_id),
        )
        if row:
            return {"name": str(row["name"]), "level": int(row["level"])}
    except Exception as e:
        logger.warning(f"_get_player_brief failed: {e}")

    return {"name": f"tg:{int(tg_id)}", "level": 0}


def _decorate_state_with_names(st: Dict[str, Any], b1: Dict[str, Any], b2: Dict[str, Any]) -> Dict[str, Any]:
    st["p1_name"] = b1.get("name")
    st["p2_name"] = b2.get("name")
    st["p1_level"] = b1.get("level", 0)
    st["p2_level"] = b2.get("level", 0)
    return st
...
async def perun_duel_state(
    ...
    p1 = int(st.get("p1") or 0)
    p2 = int(st.get("p2") or 0)

    if int(me) not in (p1, p2):
        raise HTTPException(status_code=403, detail="not_participant")

    pool = await get_pool()
    async with pool.acquire() as conn:
        b1 = await _get_player_brief(conn, p1) if p1 else {"name": "—", "level": 0}
        b2 = await _get_player_brief(conn, p2) if p2 else {"name": "—", "level": 0}

    st = _decorate_state_with_names(st, b1, b2)
    return DuelActionResponseDTO(ok=True, event="state", state=st)


async def _decorate_res_state(res: Dict[str, Any]) -> Dict[str, Any]:
    st = res.get("state") or None
    if not st:
        return res
    ...
    try:
        ...
        logger.warning(f"_decorate_res_state failed: {e}")
...
async def perun_duel_attack(
    ...
    res = await _decorate_res_state(res)
