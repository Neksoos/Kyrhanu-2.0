...
export async function runsRoutes(app: FastifyInstance) {
  app.post("/runs/start", async (req, reply) => {
    ...
    const ch = await pool.query(
      `select * from characters where user_id=$1 and day_key = (now() at time zone 'utc')::date limit 1`,
      [au.id]
    ).then((r) => r.rows[0]);
    ...
    const run = await pool.query(
      `insert into runs (id, user_id, character_id, seed, status, state, started_at)
       values (gen_random_uuid(), $1, $2, $3, 'active', $4::jsonb, now())
       returning *`,
      [au.id, ch.id, seed.toString(), JSON.stringify({ room: 0, hp_loss: 0 })]
    ).then((r) => r.rows[0]);

    // first encounter
    await pool.query(
      `insert into encounters (id, run_id, idx, kind, seed, data, created_at)
       values (gen_random_uuid(), $1, 0, 'fight', $2, $3::jsonb, now())`,
      [run.id, seed.toString(), JSON.stringify({ enemy: "Мавка-розвідниця", level: 1 })]
    );
  ...
  app.post("/runs/act", async (req, reply) => {
    const au = await app.requireAuth(req);
    const body = z.object({ run_id: z.string().uuid(), action: z.enum(["ATTACK", "DEFEND", "FLEE"]) }).parse(req.body);

    const run = await pool.query(`select * from runs where id=$1 and user_id=$2`, [body.run_id, au.id]).then((r) => r.rows[0]);
    if (!run) return reply.code(404).send({ error: "RUN_NOT_FOUND" });
    if (run.status !== "active") return reply.code(400).send({ error: "RUN_NOT_ACTIVE" });
    ...
    await pool.query(
      `insert into encounters (id, run_id, idx, kind, seed, data, created_at)
       values (gen_random_uuid(), $1, $2, 'event', (random()*1000000)::int, $3::jsonb, now())`,
      [run.id, nextRoom, JSON.stringify({ text: "Туман кургану шепоче… ти знаходиш дрібний оберіг." })]
    );
  ...
  app.post("/runs/finish", async (req, reply) => {
    const au = await app.requireAuth(req);
    const body = z.object({ run_id: z.string().uuid() }).parse(req.body);
